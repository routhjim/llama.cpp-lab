...
common_models_handler common_models_handler_init(const common_params & params, llama_example curr_ex) {
    ...
    const bool spec_type_draft_dflash = std::find(params.speculative.types.begin(),
                                        ...
                                        std::find(params.speculative.types.begin(),
                                           params.speculative.types.end(),
                                           COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH_ADAPTIVE) != params.speculative.types.end();
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--spec-draft-lookahead"}, "N",
        string_format("MTP only: draft n_max+N tokens, hand the server n_max and keep the tail as a reserve; "
                      "a fully-accepted round whose bonus token matches the reserve head then costs no drafter "
                      "forward (default: %d, 0 = off)", params.speculative.draft.n_lookahead),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("invalid value");
            }
            params.speculative.draft.n_lookahead = value;
        }
    ...
    add_opt(common_arg(
        {"-ubd", "--ubatch-size-draft"}, "N",
        string_format("physical maximum batch size of the draft context (default: %d = inherit the target's --ubatch-size); shrinks the draft compute buffers", params.speculative.draft.n_ubatch),
        [](common_params & params, int value) {
