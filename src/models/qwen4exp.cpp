...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    auto load_block = [&](int il, int flags) {
        ...
        // hot/cold split experts (fn-expert-swap): present when the GGUF came from
        // fn_split.py; the per-layer hot count is read off the tensor itself
        ggml_tensor * hot_meta = ml.get_tensor_meta(tn(LLM_TENSOR_FFN_GATE_EXPS_HOT, "weight", il).str().c_str());
        if (hot_meta != nullptr) {
            qwen4exp_force_vk_reorder_cap();
            const int64_t n_hot  = hot_meta->ne[2];
            const int64_t n_cold = n_expert - n_hot;
            GGML_ASSERT(n_hot > 0 && n_cold > 0);
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_ffn(ggml_tensor * cur, const int il) {
    ...
    if (model.layers[il].ffn_gate_exps_hot != nullptr) {
        // hot/cold split experts: route once over the full expert set, then
        // dispatch each pack with pack-local ids and coalition-normalized weights
        ...
        // NOTE: this split routing subgraph is mis-executed by a Vulkan
        // graph optimization at batch (PPL 28 vs 1.65 on wiki chunks; a
        // topk-moe-fusion pattern-breaking no-op scale did NOT help).
        // Run with GGML_VK_DISABLE_GRAPH_OPTIMIZE=1 until root-caused.
        ggml_tensor * selected_experts = ggml_argsort_top_k(ctx0, probs, n_used); // [n_used, n_tokens]
        ...
        // normalize over the full coalition (norm_w path of build_moe_ffn)
        ...
        for (int t = 0; t < 2; t++) {
            ...
            ggml_tensor * out_t = build_moe_ffn(cur,
                ...
                sel_t, w_t);
            // no-op scale: the pack-combine ADD must not directly follow the
            // pack's final reduce ADD — the Vulkan graph optimizer's ADD+ADD
            // grouping exemption assumes a fusable bias pattern and mis-executes
            // this dependent pair at batch (PPL 383 vs 1.65 on wiki chunk 1)
