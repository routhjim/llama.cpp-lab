...
llama_model_deepseek4::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (layer.ffn_gate_exps_hot != nullptr) {
            // tiered MoE: route once, dispatch per precision tier
            ...
            switch ((llama_expert_gating_func_type) hparams.expert_gating_func) {
                ...
                case LLAMA_EXPERT_GATING_FUNC_TYPE_SQRT_SOFTPLUS:
                    ...
                    probs = ggml_sqrt(ctx0, ggml_softplus(ctx0, logits)); break;
                default: GGML_ABORT("tiered moe: unsupported gating");
            }
            if (selected_experts == nullptr) {
                ggml_tensor * sel_probs = exp_probs_b
                    ? ggml_add(ctx0, probs, exp_probs_b) : probs;
            ...
            for (int t = 0; t < 3; t++) {
                ...
                ggml_tensor * out_t = build_moe_ffn(cur,
                        nullptr, nullptr,
                        tiers[t].up,   nullptr,
                        tiers[t].gate, nullptr,
                        tiers[t].down, nullptr,
                        nullptr,
...
llama_model_deepseek4::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params) :
    ...
    GGML_ASSERT(layer.ffn_gate_exps_hot == nullptr && "tiered MoE not supported in MTP block");
